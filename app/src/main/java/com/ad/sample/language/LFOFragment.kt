...
open class LFOFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding?.textView5?.text = "LFO Fragment"
        binding?.button3?.setOnClickListener {
            findNavController().navigate(R.id.action_LFOFragment_to_LFOSelectFragment)
        }
        setNativeAd()
    }
    ...
    private fun setNativeAd() {
        val nativeAdControl = activity?.let {
            NativeAdControl(
                ...
                config = NativeConfig(
                    ...
                    isShow = ConfigManager.isShowNativeLanguage,
        ...
        NativeControl.nativeLFO1.observe(viewLifecycleOwner) { ad ->
            ad?.let {
        ...
        context?.let { NativeControl.requestNativeLFO2(it) }
    }
}
