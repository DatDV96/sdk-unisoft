...
class UmpManager private constructor(private val activity: Activity) {
    companion object {
        @JvmStatic
        fun getInstance(activity: Activity): UmpManager = UmpManager(activity)
    }

    private val consentInformation by lazy { UserMessagingPlatform.getConsentInformation(activity) }
    private var umpCallback: UmpCallback? = null
     ...
     *
     * @param umpCallback: 'onResponse' used to call ads, it will be called in either success or error case
     *                          'onPolicyRequired' will indicate whether to show privacy button or not
     */

    fun initDebugConsent(deviceId: String = getDeviceId(), umpCallback: UmpCallback) {
        this.umpCallback = umpCallback
    ...
    fun initReleaseConsent(umpCallback: UmpCallback) {
        this.umpCallback = umpCallback
    ...
    private fun requestConsent(params: ConsentRequestParameters) {
        consentInformation.requestConsentInfoUpdate(activity, params, {
            ...
            if (consentInformation.isConsentFormAvailable && consentInformation.consentStatus == ConsentInformation.ConsentStatus.REQUIRED) {
                ...
                umpCallback?.onSuccess()
            }
        }, { error ->
            // Handle the error.
            Log.e("ConsentManager", "requestConsent: $error")
            umpCallback?.onSuccess(error.message)
        })
    }
    ...
    private fun loadForm() {
        ...
        UserMessagingPlatform.loadAndShowConsentFormIfRequired(activity) { formError ->
            formError?.let {
                umpCallback?.onSuccess(it.message)
            } ?: run {
                umpCallback?.onSuccess()
